...
function LanguageSwitcher() {
    ...
    return (
        <div className="relative group">
            {/* Tooltip */}
            <motion.div
                className="absolute -bottom-10 left-1/2 -translate-x-1/2 bg-[#fafafa] px-3 py-1.5 rounded-lg text-xs text-black whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-50"
                style={{
                    boxShadow: "0 4px 12px rgba(0, 0, 0, 0.3)"
                }}
            >
                {language === 'ar' ? 'Switch Language' : 'تبديل اللغة'}
                <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-[#fafafa] rotate-45"></div>
            </motion.div>

            {/* Toggle Switch Container */}
            <motion.button
                onClick={toggleLanguage}
                className="relative flex items-center gap-1 px-1 py-1 rounded-full border border-[#000000]/30 hover:border-[#000000]/60 transition-all duration-200 overflow-hidden"
                style={{
            ...
            >
                ...
                <motion.div
                    ...
                    style={{
                        ...
                        boxShadow: "0 4px 12px rgba(0, 0, 0, 0.5), 0 0 25px rgba(0, 0, 0, 0.3)",
                        width: "calc(50% - 4px)"
                    }}
                    animate={{
            ...
            <motion.div
                className="absolute -top-1 -right-1 w-5 h-5 bg-[#000000] rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                whileHover={{ scale: 1.2 }}
                animate={isClicked ? { rotate: 360 } : { rotate: 0 }}
                transition={{ duration: 0.5 }}
            >
                <Languages className="w-3 h-3 text-[#ffffff]" />
            </motion.div>
        </div>
    );
}
